...
function get_cart($input) {
    ...
    if (empty($input["user_id"])) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $stmt = $pdo->prepare("SELECT title, actors, director, price, video
                                          FROM user_cart
                                          JOIN movies ON user_cart.movie_id = movies.id
                                          WHERE user_cart.cart_id = ?");
...
function add_cart($input) {
    ...
    if (empty($input["user_id"])) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $cart_id = $stmt->fetch()["id"];
            $stmt = $pdo->prepare("INSERT INTO user_cart (cart_id, movie_id) VALUES (?, ?)");
